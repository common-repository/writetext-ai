...
$total_items          = count( $competitor_keywords );
...
$sort_filter_data           = wtai_get_keyword_analysis_sort_filter( $record_id, 'competitor', $record_type );
...
<div class="wtai-keyword-table-parent-wrap wtai-has-filter-wrap" >
	<table class="wtai-keyword-table wtai-has-filter wtai-keyword-table-competitor-keywords wtai-has-rank-intent-data" 
		data-keyword-type="competitor" data-sort-field="<?php echo esc_attr( $sort_type_selected ); ?>" 
								...
								<div class="wtai-difficulty-filter-wrap" >
									...
									<div class="wtai-difficulty-filter-options wtai-sort-idea-options" >
										...
										foreach ( $difficulty_filters as $filter_id => $filter_label ) {
											...
											echo '<div>
													<label>
														<input type="checkbox" ' . esc_attr( $cb_checked ) . ' id="wtai-difficulty-filter-' . esc_attr( $filter_id ) . '" name="wtai_difficulty_filter_competitor[]" class="wtai-sort-idea-filter-input wtai-difficulty-filter wtai-difficulty-filter-' . esc_attr( $filter_id ) . '" value="' . esc_attr( wp_unslash( $filter_id ) ) . '" /> ' . wp_kses_post( $filter_label ) . '
													</label>
													</div>';
										}
										?>
								...
								<div class="wtai-volume-filter-wrap" >
									...
									<div class="wtai-volume-filter-options wtai-sort-idea-options" >
										...
										foreach ( $volume_filters as $filter_id => $filter_label ) {
											...
											echo '<div>
														<label>
															<input type="radio" ' . esc_attr( $cb_checked ) . ' id="wtai-volume-filter-' . esc_attr( $filter_id ) . '" name="volume_filter_competitor" class="wtai-sort-idea-filter-input wtai-volume-filter wtai-volume-filter-' . esc_attr( $filter_id ) . '" value="' . esc_attr( wp_unslash( $filter_id ) ) . '" /> ' . wp_kses_post( $filter_label ) . '
														</label>
													</div>';
										}
										?>
		...
		<tbody class="wtai-post-data wtai-keyword-tbody" data-postfield="keyword_competitor_table" >
			<?php
			$item_ctr = 1;
			foreach ( $competitor_keywords as $keyword_data ) {
				...
				if ( $top_serp_infos ) {
					...
					$serp_info_html     = wtai_get_keyword_serp_html( $record_id, $keyword, 'competitor', $top_serp_infos, $serp_date );
				}
				...
				<tr class="wtai-has-data wtai-keyword-tr <?php echo wp_kses_post( $tr_class ); ?>" 
					...
					<td class="wtai-col-volume wtai-col-normal-mobile" data-filter-value="<?php echo wp_kses_post( $search_vol ); ?>" data-value="<?php echo wp_kses_post( $search_vol ); ?>" ><?php echo wp_kses_post( $search_vol ); ?></td>
					...
					if ( $keyword_added ) {
						?>
							...
							data-keyword-type="competitor" data-type="remove" 
